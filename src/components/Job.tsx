...
export default function Job({ job, handleClick }: Props) {
  ...
  return (
    <motion.li
      ...
      key={job.id}
      className={`job | shadow-regular radius-regular flow ${
        priority && "priority"
      }`}
      data-flow="sm"
    >
      ...
      <div className="flow" data-flow="sm">
        <div className="flex">
          ...
          {job.new && (
            <span className="new | badge | text-white bg-primary">New!</span>
          )}
          {job.featured && (
            <span className="featured | badge | text-white bg-neutral-200">
              Featured!
            </span>
          )}
        </div>
        <div className="flow" data-flow="sm">
          <h2 className="position | fw-bold">{job.position}</h2>
          <div className="text-neutral-500 flex">
            <span className="postedAt">{job.postedAt}</span>
            <span
              className="text-neutral-500"
              style={{ transform: "translateY(-20%)", fontSize: "28px" }}
            >
              .
            </span>
